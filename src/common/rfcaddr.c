 ...
 *
 * $Id: rfcaddr.c,v 4.5 1998/05/03 12:46:37 mj Exp $
 *
...
RFCAddr rfcaddr_from_ftn(char *name, Node *node)
{
	...
	if(node->zone == -1)
	    ...
	    BUF_COPY(rfc.addr, ftn_to_inet(node, FALSE));
...
/*
 * rfcaddr_to_asc() --- Text representation of RFCAddr
 */
char *rfcaddr_to_asc(RFCAddr *rfc, int real_flag)
